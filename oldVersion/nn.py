...
def backward_softmax(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    x = forward_softmax(x)
    temp = np.diag(x) - x.reshape(-1,1).dot(x.reshape(1,-1))
    return grad_outputs.dot(temp)
    # *** END CODE HERE ***
...
def backward_relu(x, grad_outputs):
    ...
    """
    # *** START CODE HERE ***
    dZ = np.array(grad_outputs, copy=True)
    dZ[x<=0] = 0
    return dZ
    # *** END CODE HERE ***
    ...
    for x in range(input_width - conv_width + 1):
        for y in range(input_height - conv_height + 1):
            for output_channel in range(conv_channels):
                output[output_channel, x, y] = np.sum(np.multiply(data[:, x:(x + conv_width), y:(y + conv_height)],\
                    conv_W[output_channel, :, :, :])) + conv_b[output_channel]
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    gradient_bias = np.sum(output_grad)           
    
    gradient_weights = np.zeros_like(conv_W)

    #rotate the convolution kernal by 180 degrees
    #and pad the output_grad in preparation for convolution
    conv_W = conv_W[:,:,::-1,::-1]
    
    #padding
    l = []
    for i in range((output_grad.shape[0])):
        l.append(np.pad(output_grad[i,:,:], CONVOLUTION_SIZE-1))
    for i in range(len(l)):
        l[i] = l[i].reshape(1, l[i].shape[0], l[i].shape[1])
    output_grad = np.concatenate(l, axis=0)
    

    output = np.zeros([2,data.shape[1],data.shape[2]])
    
    for x in range(input_width):
        for y in range(input_height):
            for output_channel in range(conv_channels):
                output[output_channel, x, y] = np.sum(np.multiply(output_grad[output_channel,x:(x+conv_width),y:(y+conv_height)], \
                    conv_W[output_channel, :, :, :] ))
    
    for x in range(conv_width):
        for y in range(conv_height):
            for output_channel in range(conv_channels):
                gradient_weights[output_channel,0,x, y] = np.sum(np.multiply(output_grad[:, x:(x+input_width), y:(y+input_height)],\
                     output[output_channel, :,:]))

    return gradient_weights ,gradient_bias ,output 
    # *** END CODE HERE ***
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    for i in range(0, input_height, pool_height):
        for j in range(0, input_width, pool_width):
           for k in range(input_channels):
                index = np.unravel_index(data[k, i:i+pool_height, j:j+pool_width].argmax(), shape=(pool_width, pool_height))
                index_i = index[0]
                index_j = index[1]
                index_i += pool_height
                index_j += pool_width
                gradients[k,index_i, index_j] = output_grad[k, index_i % pool_height, index_j % pool_width]
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    gradient_weights = data.reshape(-1,1).dot(output_grad.reshape(1,-1))
    gradient_bias = output_grad
    gradient_data = output_grad.dot(weights.T)
    return gradient_weights, gradient_bias, gradient_data
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params):
    ...
    #############################################
    cost_after_cross_entropy = backward_cross_entropy_loss(y, labels)
    after_backward_softmax = backward_softmax(logits ,cost_after_cross_entropy)
    gradient_W2, gradient_b2 ,after_backward_linear= backward_linear(W2, b2, flattened,after_backward_softmax)

    unflattened = after_backward_linear.reshape(CONVOLUTION_FILTERS, MAX_POOL_SIZE, MAX_POOL_SIZE)
    after_backward_relu = backward_relu(first_max_pool, unflattened)
    after_backward_max_pool = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, after_backward_relu)
    gradient_W1 ,gradient_b1 ,after_backward_convolution = backward_convolution(W1, b1, data, after_backward_max_pool)
    
    gradients = {'W1':gradient_W1,
        'b1':gradient_b1,
        'W2':gradient_W2,
        'b2':gradient_b2
     }
...
def nn_train(
    ...
    for batch in range(num_batches):
        ...
        # backward propagation and gradient descent to change the parameters
...
def run_train(all_data, all_labels, backward_prop_func):
    '''
    get _initial_params = randomized matrix
    forward_prop = convolution + relu + maxpool + flatten + linear + softmax
    backward_prop = 
    '''
...
# plot loss and accuracy by time and save figure 
...
#split the training set into dev_ and train_
...
#two dictionary containing the data
    all_data = {
        'train': train_data,
        'dev': dev_data,
    }
